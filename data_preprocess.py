...
def load_dataset(return_file=True):
    # Load train data
    X_train = utils.load_json(CONFIG_DATA['train_set_path'][0])
    y_train = utils.load_json(CONFIG_DATA['train_set_path'][1])

    # Load valid data
    X_valid = utils.load_json(CONFIG_DATA['valid_set_path'][0])
    y_valid = utils.load_json(CONFIG_DATA['valid_set_path'][1])

    # Load test data
    X_test = utils.load_json(CONFIG_DATA['test_set_path'][0])
    y_test = utils.load_json(CONFIG_DATA['test_set_path'][1])
...
def generate_preprocessor(return_file=False):
    ...
    # Load data
    X = utils.load_json(CONFIG_DATA['train_set_path'][0])
    y = utils.load_json(CONFIG_DATA['train_set_path'][1])
...
def preprocess_data(type, return_file=False):
    ...
    # Load data
    X = utils.load_json(CONFIG_DATA[f'{type}_set_path'][0])
    y = utils.load_json(CONFIG_DATA[f'{type}_set_path'][1])
    ...
    # Dump file
    utils.dump_json(X_clean, CONFIG_DATA[f'{type}_clean_path'][0])
    utils.dump_json(y_clean, CONFIG_DATA[f'{type}_clean_path'][1])
