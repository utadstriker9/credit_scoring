...
def train_model(return_file=True):
    ...
    # Load dataset
    X_train = utils.load_json(CONFIG_DATA['train_clean_path'][0])
    y_train = utils.load_json(CONFIG_DATA['train_clean_path'][1])
    X_valid = utils.load_json(CONFIG_DATA['valid_clean_path'][0])
    y_valid = utils.load_json(CONFIG_DATA['valid_clean_path'][1])
